...
import {
  DocumentSync,
  type SyncStatus,
} from "../src/lib/document-sync.svelte.ts"
...
describe("DocumentSync", () => {
  test("moves from offline through syncing to synced and cleans up", async () => {
    ...
    const sync = new DocumentSync(repo as unknown as Repo, handle)
    let status: SyncStatus | undefined
    const stop = observe(() => sync.status, value => (status = value))
    flushSync()
  ...
  test("flushes a newer change before reporting synced", async () => {
  ...
  test("reports failures and retries after another change", async () => {
    const repo = new FakeRepo(true)
    const handle = new FakeHandle()
    const sync = new DocumentSync(repo as unknown as Repo, handle)
    let observed: { status: SyncStatus; error: unknown } | undefined
    const stop = observe(
      () => ({ status: sync.status, error: sync.error }),
  ...
  test("ignores completion after cleanup and can resubscribe", async () => {
    const repo = new FakeRepo(true)
    const handle = new FakeHandle()
    const sync = new DocumentSync(repo as unknown as Repo, handle)
    let status: SyncStatus | undefined
    const firstStop = observe(() => sync.status, value => (status = value))
    flushSync()
    ...
    const secondStop = observe(() => sync.status, value => (status = value))
    flushSync()
    expect(repo.flushes).toHaveLength(2)
    expect(status).toBe("syncing")
    await repo.resolveFlush(1)
...
class FakeHandle {
  readonly documentId = "test-document" as never
